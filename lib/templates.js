...
function generateNav(pages) {
  ...
  return function navFor(currentOutputPath) {
    ...
    for (const [dir, label] of Object.entries(DIR_LABELS)) {
      ...
      let dirPages = [];
      for (const [sectionDir, sectionPages] of Object.entries(sections)) {
        if (sectionDir === dir || sectionDir.startsWith(dir + '/')) {
          dirPages = dirPages.concat(sectionPages);
        }
      }
